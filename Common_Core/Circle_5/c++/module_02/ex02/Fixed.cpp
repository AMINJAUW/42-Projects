...
/*   Updated: 2023/11/21 12:42:38 by aminjauw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (INT_MAX - this->_raw_bits < rhs.getRawBits())
		cout << "Carefull You are overflowing" << endl;
...
Fixed	Fixed::operator-(Fixed const & rhs) const
{
	if (INT_MIN + this->_raw_bits > rhs.getRawBits())
		cout << "Carefull You are undeflowing" << endl;
	float result = (this->_raw_bits - rhs.getRawBits()) / (1 << _frac_bits);
	return Fixed(result);
}
...
Fixed	Fixed::operator*(Fixed const & rhs) const
{
	if (INT_MAX / this->toFloat() < rhs.toFloat())
		cout << "Carefull You are overflowing" << endl;
