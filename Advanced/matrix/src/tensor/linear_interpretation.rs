...
mod tests {
    ...
    fn test_lerp_complex_vector() {
        let u = Tensor::new(vec![2], vec![Complex { re: 1.0, im: 1.0 }, Complex { re: 2.0, im: 2.0 }]);
        let v = Tensor::new(vec![2], vec![Complex { re: 3.0, im: 3.0 }, Complex { re: 4.0, im: 4.0 }]);

        let t = 0.5;
        let result = lerp(u.clone(), v.clone(), t);
        let expected = Tensor::new(vec![2], vec![Complex { re: 2.0, im: 2.0 }, Complex { re: 3.0, im: 3.0 }]);
    ...
    fn test_lerp_f32_matrix() {
        let u = Tensor::new(vec![2, 3], vec![
            1.0f32, 2.0f32, 3.0f32,
            4.0f32, 5.0f32, 6.0f32
        ]);
        let v = Tensor::new(vec![2, 3], vec![
            7.0f32, 8.0f32, 9.0f32,
            10.0f32, 11.0f32, 12.0f32
        ]);

        let t = 0.5;
        let result = lerp(u.clone(), v.clone(), t);
        let expected = Tensor::new(vec![2, 3], vec![
            4.0f32, 5.0f32, 6.0f32,
            7.0f32, 8.0f32, 9.0f32
        ]);
    ...
    fn test_lerp_complex_matrix() {
        ...
        let v = Tensor::new(vec![2, 3], vec![
            Complex { re: 7.0, im: 7.0 }, Complex { re: 8.0, im: 8.0 }, Complex { re: 9.0, im: 9.0 },
            Complex { re: 10.0, im: 10.0 }, Complex { re: 11.0, im: 11.0 }, Complex { re: 12.0, im: 12.0 }
        ]);
